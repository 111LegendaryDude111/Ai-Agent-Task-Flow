...
2. If invoked through `/auto-flow`, load `context/core/workflows/auto-flow.md`, `context/core/workflows/auto-flow-state-machine.json`, and `skills/tdd/SKILL.md`; follow only allowed state transitions.
...
6. For feature/bugfix implementation, enforce `skills/tdd/SKILL.md`: `TestDesigner` creates or confirms one focused RED behavior test before production code.
...
- Use vertical TDD for feature/bugfix work: one behavior → one failing test → minimal implementation → green → optional scoped refactor → green.
